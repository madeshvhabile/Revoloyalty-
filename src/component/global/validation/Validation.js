export default class Validation {
    ...
    nestedValidation(obj, map) {
        ...
        let checkMin=(prop,min)=>{
            if(obj.hasOwnProperty(prop)){
                // var fieldva=obj[prop].toString()
                if(!emptyField(prop)){
                    var count =obj[prop].toString().length
                    if(count > min-1){
                        return false
                    }
                }
                ...
                for (let i in asserts) {
                    if (asserts[i] === 'not_blank') {
                        if (emptyField(prop)) {
                            ...
                            if(prop == 'email'){
                                pushError(prop, 'Enter Valid EmailId');
                            }else
                            pushError(prop, 'front_error.not_blank');
                        }
                    }
                    if(asserts[i].match(/^min/) ){
                        let min =parseInt(asserts[i].split('-')[1]);
                       if (checkMin(prop,min)){
                        pushError(prop, 'This field Must be minimum '+ min +' characters');
                    }
                }
                if(asserts[i].match(/^max/) ){
                    let min =parseInt(asserts[i].split('-')[1]);
                   if (checkMax(prop,min)){
